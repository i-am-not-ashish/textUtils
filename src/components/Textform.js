...
export default function TextForm(props) {
  ...
  return (
    ...
    <div className='container' style={{color: props.mode==='dark'?'white':'black'}}>
        ...
        <button disabled= {text.length===0} className="btn btn-primary mx-2 my-1" style={{backgroundColor: props.mode==='dark'?props.btnColor:'#0d6efd'}} onClick={handleClick}>Convert to UpperCase</button>
        <button disabled= {text.length===0} className="btn btn-primary mx-2 my-1" style={{backgroundColor: props.mode==='dark'?props.btnColor:'#0d6efd'}} onClick={handleDOwnClick}>Convert to LowerCase</button>
    </div>
